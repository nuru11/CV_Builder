...
function App() {
  ...
  const pasteData = (data) => {
    const newData = {
      ...
      arabicDegree: data.arabicDegree || '',
      englishDegree: data.englishDegree || '',
      ownPhoneNumber: data.ownPhoneNumber || '',
      contactPhoneNumber: data.contactPhoneNumber || '',
      monthlysalarySaudi: data.monthlysalarySaudi || '',
      monthlysalaryJordan: data.monthlysalaryJordan || ''
    };
  ...
  return (
    <div>
      <button onClick={() => pasteData({ name: 'John Doe', surname: 'john@example.com' })}>
        Paste Example Data
      </button>
